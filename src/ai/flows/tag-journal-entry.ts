 ...
 * 
 * - character Limits: Input is virtually unlimited (1M+ tokens).
...
const TagJournalEntryOutputSchema = z.object({
  ...
  flag_for_synthesis: z.boolean().describe('True if entry contains a strong signal for tomorrow’s synthesis.'),
});
...
const prompt = ai.definePrompt({
  ...
  prompt: `
    SYSTEM:
    PROMPT 00: IDENTITY
    You are Jeiva's tagging engine. You process raw user data into structured insights.
    Tone: Invisible, factual, clinical-grade precision without being clinical in language.

    LOGIC:
    1. Extract up to 3 tags from the allowed list.
    2. Create a 5–7 word neutral summary title.
    3. Sentiment: positive / neutral / low.
    4. CULTURAL FLUENCY: If an Indian food item is mentioned (e.g., Dal, Roti, Chawal), identify it explicitly.
    
    STRICT RULES:
    - Preserve the user's words exactly in any extraction.
    - DO NOT rewrite, improve, or editorialize the entry.
    - Summary must be factual, not a critique or improvement.
    
    flag_for_synthesis: set to true ONLY if the entry contains a high-tier signal (e.g., severe pain, persistent low mood, cycle start, new medication).
